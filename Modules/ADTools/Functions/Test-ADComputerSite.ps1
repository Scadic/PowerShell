Function Test-ADComputerSite
{
    ...
    (
        [Parameter(
            Mandatory         = $True,
            HelpMessage       = "Which to site to check if the computer is in.",
            Position          = 0,
            ValueFromPipeline = $False
            )
        ]
        [ValidateScript({(Get-ADReplicationSite -Filter * | Select-Object -ExpandProperty Name) -IContains $_})]
        [Alias("ADSite")]
        [Alias("Site")]
    ...
    Process
    {
        If ($ComputerName.Count -Eq 1 -And $ComputerName[0] -Eq $env:COMPUTERNAME)
        {
            ...
            If ($Object.Site -Eq $Object.TestSite){$Object.TestSucceeded = $True}
